...
- **Codex Sonnet 4.6 or Opus 4.6** (already Lextract's stack) via API
...
2. API call to Codex: extract 5 key insights -> generate 5 x 5-minute scripts + metadata (captions, hashtags, B-roll prompts)
...
1. Set up Codex API connection in middleware
...
- Lextract already uses Codex Sonnet 4.6 in its product -> same API key works for the pipeline
