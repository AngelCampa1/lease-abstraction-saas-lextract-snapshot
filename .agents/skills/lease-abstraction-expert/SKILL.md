...
**Common extraction challenges:**
- Rent may be stated as annual, monthly, or per-RSF-per-year. Normalize to annual for `base_rent_annual`. If stated as "$2.04 per month per RSF" on 5,000 RSF, annual = $2.04 * 12 * 5,000 = $122,400. But remember: Codex extracts the stated figures, Python computes the annual total.
...
All formulas are executed in the Python validation layer. Codex extracts the parameters; Python does the math.
