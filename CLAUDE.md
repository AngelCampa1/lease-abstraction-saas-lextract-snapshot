...
Dedicated production E2E credentials are stored only in an ignored local env file. Use `LEXTRACT_E2E_EMAIL`, `LEXTRACT_E2E_PASSWORD`, `LEXTRACT_E2E_BASE_URL`, and `LEXTRACT_E2E_API_URL`; never commit the actual values.
...
### Git Discipline
- **Use worktrees for all parallel agent work.** Create them inside this repo under `.worktrees/<branch-name>`, and do not work directly on `main`.
...
**Worktree isolation.** All feature/fix work MUST happen inside a git worktree under `.worktrees/<branch-name>` in this repo. Use the `using-git-worktrees` skill to create one before writing any code.
...
- `@ventora/ai-cs` ^0.6.0 — in-app AI support widget (session/chat/escalation via `/api/ai-cs` BFF)
